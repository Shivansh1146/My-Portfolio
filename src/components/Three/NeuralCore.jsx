...
export default function NeuralCore() {
  ...
  useFrame((state, delta) => {
    ...
    // 0.0 to 0.25 = Intro to Hero (scales up, moves right)
    // 0.25 to 0.5 = Hero to Skills (scales down to 0, moves left)
    ...
    if (offset < 0.25) {
      // Intro -> Hero
      const localOffset = offset * 4; // 0 to 1 over first 1/4th
      targetScale = 1 + localOffset * 0.8;
      targetX = localOffset * 2.5;
      targetY = -localOffset * 1.5;
    } else {
      // Hero -> Skills
      const localOffset = (offset - 0.25) * 4; // 0 to 1 over next 1/4th
